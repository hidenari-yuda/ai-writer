...
def parse_search_page(result):
    data = re.findall(
        r'window.mosaic.providerData\["mosaic-provider-jobcards"\]=(\{.+?\});', result.content)
...
async def scrape_search(client: ScrapflyClient, query: str, location: str):
    ...
    total_results = sum(category["jobCount"]
                        for category in data_first_page["meta"])
...
async def scrape_jobs(client: ScrapflyClient, job_keys: List[str]):
    """scrape job page"""
    urls = [
        f"https://www.indeed.com/m/basecamp/viewjob?viewtype=embedded&jk={job_key}" for job_key in job_keys]
    scraped = []
