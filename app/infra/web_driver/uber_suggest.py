...
from domain.utils.csv import write_csv
...
class UberSuggest(WebDriver):
    ...
    def sign_in(self):
        ...
        email_input = WebDriver.driver.find_element(By.NAME, 'email')
        email_input.clear()
        ...
        password_input = WebDriver.driver.find_element(By.NAME, 'password')
        password_input.clear()
        password_input.send_keys(config.config.UBER_SUGGEST_PASSWORD)
        password_input.submit()

        button = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '.sc-gSYCTC chVsAt')
        button.click()

        time.sleep(5)

        WebDriver.get('https://app.neilpatel.com/ja/labs/ai_writer')

        time.sleep(5)

        first_button = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '.sc-khIgXV')
        print("text is", first_button.text)
    ...
    def generate_meta_title(self, keyword):
        ...
        keyword_input = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '.sc-bVDCDz')
        keyword_input.clear()
        keyword_input.send_keys(keyword)
        # keyword_input.submit()

        second_button = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '.sc-eBnaGZ')
        second_button.click()

        time.sleep(5)

        copy_button = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '#copyToolClipboard')
        copy_button.click()
    ...
    def sign_out(self):
        sign_out_button = WebDriver.driver.find_element(
            By.CSS_SELECTOR, '.sc-bnFUrP')
